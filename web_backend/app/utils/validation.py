...
def validate_csv_file(file_path: str) -> list[str]:
    """Validate the EMS CSV file and return a list of error messages."""
    errors: list[str] = []
    with open(file_path, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        # Row validation
        for i, row in enumerate(reader, 2):  # Row numbers start at 2
            # Required field checks
            if not row['INCIDENT_KEY']:
                errors.append(f"Row {i}: Missing INCIDENT_KEY")

            # Date/time validation
            inc_date = row['INC_DATE']
            if not DATE_PATTERN.match(inc_date):
                errors.append(f"Row {i}: Invalid INC_DATE format '{inc_date}' (MMDDYYYY required)")

            for field in ['ARRIVAL', 'TRANSPORT']:
                value = row[field]
                if value and not DATETIME_PATTERN.match(value):
                    errors.append(f"Row {i}: Invalid {field} format '{value}' (MMDDYYYYHHmm required)")

            # Numeric field validation
            age = row['AGE']
            if age and not NUMERIC_PATTERN.match(age):
                errors.append(f"Row {i}: Invalid AGE value '{age}' (numeric/decimal required)")

            # Coded field validation
            arrest = row['ARREST']
            if arrest not in VALID_ARREST_VALUES:
                errors.append(f"Row {i}: ARREST must be 1, 2, or empty (found '{arrest}')")

            pulse = row['PULSE']
            if pulse not in VALID_PULSE_VALUES:
                errors.append(f"Row {i}: Invalid PULSE value '{pulse}'")
